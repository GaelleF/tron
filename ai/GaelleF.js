...
const calcScore = (cardinal, x, y) => {
  let freeCaseCount = 0
  while (inBounds(y) && inBounds(x) && isFree({ x, y })) {
    freeCaseCount++
    if (cardinal === NORTH) {
      y--
...
const update = state => {
  ...
  const pickedPosition = available[0]

  for (const coord of available) {
    coord.score = calcScore(coord.cardinal, coord.x, coord.y)
  }
